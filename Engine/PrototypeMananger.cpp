...
namespace nu
{
	bool PrototypeManager::Load(const rapidjson::Value& prototypes)
	{
		if (!prototypes.IsObject())
		{
			std::cerr << "Prototype data is not an object."
				<< std::endl;
	...
	std::unique_ptr<Actor> PrototypeManager::Instantiate(
	...
	) const
	{
		...
		const auto& prototype = a_prototypes[name.c_str()];

		if (!prototype.IsObject())
		{
			std::cerr << "Prototype is not an object: "
				<< name << std::endl;
		...
		//{
		//	actor->SetTag(prototype["tag"].GetString());
		//}
		...
		{
			const auto& transform = prototype["transform"];
			...
			{
				position.x = transform["position"][0].GetFloat();
				position.y = transform["position"][1].GetFloat();
			}

			if (transform.HasMember("rotation") &&
				transform["rotation"].IsNumber())
			{
				rotation = transform["rotation"].GetFloat();
			}

			if (transform.HasMember("scale") &&
				transform["scale"].IsNumber())
			{
				scale = transform["scale"].GetFloat();
			}
			...
			prototype["components"].IsArray())
		{
				...
				++i)
			{
				...
				{
					std::cerr << "Invalid component in prototype: "
						<< name << std::endl;
				...
				{
					std::cerr << "Component is missing a type in prototype: "
						<< name << std::endl;
				...
				{
					std::cerr
						<< "EnemyAI requires a player reference "
						<< "and will be handled when enemies are spawned."
						<< std::endl;
				}
				else
				{
		...
		std::cout << "Instantiated prototype: "
			<< name << std::endl;
